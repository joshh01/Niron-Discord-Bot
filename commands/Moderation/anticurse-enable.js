...
module.exports = {
    ...
    aliases: ['acenable', 'antiswear-enable'],
    cooldown: 0,
    permissions: ['ADMINISTRATOR'],
    usage: "anticurse-enable",
    description: "Enables anticurse.",
    async execute(message, args, client, Discord) {
        ...
        if (await quickmongo.fetch(`swear-${message.guild.id}`) === null) {
            ...
            message.channel.send('Anticurse has been **ENABLED.**');
            if (logsChannelCheck) {
                if (!logsCheck) return;
                if (!acCheck) return;
        ...
        } else return message.channel.send('Anticurse has **ALREADY BEEN ENABLED.**');
    }
}